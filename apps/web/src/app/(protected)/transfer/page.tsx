...
import { getAccounts, createTransfer } from '@/lib/api/requests';
import type { Account, CreateTransferBody } from '@/lib/api/types';
...
export default function TransferPage() {
  ...
  useEffect(() => {
    getAccounts()
      .then((res) => {
        ...
        setAccounts(list);
        if (list.length > 0 && !form.fromAccountId)
          setForm((f) => ({ ...f, fromAccountId: list[0].id }));
      })
  ...
  const validate = (): boolean => {
    ...
    if (
      ...
      e.toAccountId = 'Source and destination must be different';
